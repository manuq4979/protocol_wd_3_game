        ...
        for char in chars:
            if char[0] == "HP":
                ...
                if ch < 0: # если вдруг будут применены характеристики другого персонажа, а потом сняты после того как HP подбили, может тоже быть отрицательное значени, наверное.
                    ch = 0
                if ch > 100: # HP менчется этим кодом лишь если был экипирован инструмент или если был вызван иной метод использующий этот! hp не должно повышаться инструментом выше чем на 100 едениц!!! Таким образом хилки не будут повышать дефолтное HP.
                    ch = 100
                prof.set_HP(ch)
            if char[0] == "armor":
                if up == False:
