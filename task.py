    ...
    def __init__(self, title="", description="", complexity="+", activation_time="", reward="", status="Active"):
        ...
        if activation_time != '' and type(activation_time) != int:
            try:
                ...
                # вкроятно причино бага стало то, что раньше это не конвертировалось в дату, а было числом - то, через сколько дней сброс!
...
class HabitTask(Task):
    ...
    def check_reset_time(self):
        ...
        current_date = str(time_localization_function.current_datetime.date())
